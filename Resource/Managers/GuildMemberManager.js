...
module.exports = {
    "receive": function (client, payload) {
        ...
        if (Event == "GUILD_MEMBER_ADD") {
            ...
            client._guilds[Data.guild_id]._data.members._members.push(guildMember)
            client.emit("guildmemberadd", guildMember);
        } else {
            // Guild Member Bye Bye
            client._guilds[Data.guild_id]._data.members._members = client._guilds[Data.guild_id]._data.members._members.filter(member => member.id != Data.user.id);
